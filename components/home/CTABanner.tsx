import FadeUp from '@/components/ui/FadeUp'
import MagneticButton from '@/components/ui/MagneticButton'
import type { HomepageData } from '@/sanity/lib/types'

export default function CTABanner({ data }: { data: HomepageData | null }) {
  return (
    <section className="bg-light py-28 lg:py-44 px-6 lg:px-10 overflow-hidden">
      <div className="max-w-[1440px] mx-auto">

        <FadeUp>
          <div className="flex items-center gap-3 mb-14">
            <span className="w-4 h-px bg-dark/25" />
            <span className="text-[10.5px] uppercase tracking-[0.24em] text-dark/30 font-medium">Let's Work Together</span>
          </div>
        </FadeUp>

        <div className="grid grid-cols-1 lg:grid-cols-[1.2fr_1fr] gap-16 lg:gap-28 items-end">
          <FadeUp>
            <h2
              className="font-[200] text-dark leading-[0.86] tracking-[-0.045em]"
              style={{ fontSize: 'clamp(44px, 6vw, 88px)' }}
            >
              {data?.ctaBannerHeading || "Let's build something extraordinary."}
            </h2>
          </FadeUp>

          <div>
            <FadeUp delay={0.1}>
              <p className="text-dark/40 text-[14px] leading-[1.85] font-light max-w-sm">
                {data?.ctaBannerParagraph || "We're not here to sell you a package. We're here to build the future of your business. Ditch the off-the-shelf."}
              </p>
            </FadeUp>
            <FadeUp delay={0.2}>
              <div className="flex items-center gap-5 mt-10">
                <MagneticButton
                  href="/contact-us"
                  className="group inline-flex items-center gap-2.5 bg-dark hover:bg-orange text-white text-[12.5px] font-medium px-7 py-3.5 rounded-full transition-all duration-300"
                >
                  Contact Us
                  <span className="text-[10px] transition-transform duration-200 group-hover:translate-x-px group-hover:-translate-y-px">↗</span>
                </MagneticButton>
                <MagneticButton
                  href="/get-a-quote"
                  className="text-[12.5px] text-dark/35 hover:text-dark transition-colors"
                >
                  Get a quote →
                </MagneticButton>
              </div>
            </FadeUp>
          </div>
        </div>

      </div>
    </section>
  )
}
