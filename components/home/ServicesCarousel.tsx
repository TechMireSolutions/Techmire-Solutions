...
const FALLBACK: Service[] = [
  ...
  { _id: '8', title: 'Branding', shortDescription: 'Strategic identities that last.', slug: { current: 'branding' }, parentCategory: 'top-level', order: 8 },
]
...
export default function ServicesCarousel({ heading, services }: { heading: string; services: Service[] }) {
  ...
  return (
    <section className="bg-light py-28 lg:py-40 px-6 lg:px-10">
          ...
          {list.map((s, i) => {
            ...
            return (
              ...
              >
                ...
                <motion.span
                  animate={{ opacity: isHov ? 0.06 : 0, x: isHov ? 0 : 30 }}
                  transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1] }}
                  className="absolute right-16 top-1/2 -translate-y-1/2 font-[200] text-white pointer-events-none select-none"
                  style={{ fontSize: 'clamp(100px, 14vw, 200px)', lineHeight: 1, letterSpacing: '-0.05em' }}
                  aria-hidden
                >
                  {String(i + 1).padStart(2, '0')}
                </motion.span>

                <Link href={href} className="relative group flex items-center justify-between py-5 border-b border-dark/[0.07]">
                  <div className="flex items-center gap-8 lg:gap-14">
                    <span className={`text-[10px] font-medium tabular-nums w-4 shrink-0 transition-colors duration-300 ${isHov ? 'text-white/25' : 'text-dark/18'}`}>
                      {String(i + 1).padStart(2, '0')}
                    </span>
                    <span
                  ...
                  <div className="flex items-center gap-5 lg:gap-8">
                    <span className={`hidden lg:block text-[12px] font-light max-w-[240px] line-clamp-1 transition-colors duration-300 ${isHov ? 'text-white/35' : 'text-dark/25'}`}>
                      {s.shortDescription}
                    </span>
                    <motion.div
                      animate={{
                        ...
                        color: isHov ? '#080808' : 'rgba(0,0,0,0.2)',
                      }}
                      transition={{ duration: 0.3 }}
                      className="w-9 h-9 rounded-full border flex items-center justify-center text-[11px] shrink-0"
                    >
                      ↗
                    </motion.div>
                  </div>
