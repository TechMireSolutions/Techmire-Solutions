...
export default async function ContactPage() {
  const pageData: ContactPageData | null = await client.fetch(contactPageQuery).catch(() => null)

  return (
    <>
      <InnerPageHero
        title={pageData?.hero?.title || "Contact Us"}
        subtitle={pageData?.hero?.subtitle || "Whether you have a question, a project idea, or just want to say hi — we're all ears."}
        overline={pageData?.hero?.overline || "Say Hello"}
      />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 max-w-[1200px] mx-auto">
          <div>
            ...
            <div className="mt-12 space-y-5 border-t border-white/[0.06] pt-10">
              {[
                { label: 'Address', value: pageData?.locations?.[0]?.address || 'R-591, F.B Area Block 20, Karachi, Sindh' },
                { label: 'Phone', value: pageData?.locations?.[0]?.phone || '(+92) 317 222 5152  ·  (+44) 7724 697972' },
                { label: 'Email', value: pageData?.emails?.general || 'connect@techmiresolutions.com' },
              ].map((item) => (
